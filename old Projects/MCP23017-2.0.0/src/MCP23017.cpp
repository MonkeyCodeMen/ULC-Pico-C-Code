...
MCP23017::MCP23017(u8_t address, TwoWire& bus) {
	_deviceAddr = address;
	_bus = &bus;
}
...
void MCP23017::portMode(MCP23017Port port, u8_t directions, u8_t pullups, u8_t inverted)
{
...
void MCP23017::pinMode(u8_t pin, u8_t mode, bool inverted)
{
...
void MCP23017::digitalWrite(u8_t pin, u8_t state)
{
...
u8_t MCP23017::digitalRead(u8_t pin)
{
	MCP23017Register gpioreg = MCP23017Register::GPIO_A;
	u8_t gpio;
	if(pin > 7)
...
void MCP23017::writePort(MCP23017Port port, u8_t value)
{
	writeRegister(MCP23017Register::GPIO_A + port, value);
}

void MCP23017::write(u16_t value)
{
	writeRegister(MCP23017Register::GPIO_A, lowByte(value), highByte(value));
}

u8_t MCP23017::readPort(MCP23017Port port)
{
...
u16_t MCP23017::read()
{
	u8_t a = readPort(MCP23017Port::A);
	u8_t b = readPort(MCP23017Port::B);

	return a | b << 8;
}

void MCP23017::writeRegister(MCP23017Register reg, u8_t value)
{
	_bus->beginTransmission(_deviceAddr);
	_bus->write(static_cast<u8_t>(reg));
	_bus->write(value);
	_bus->endTransmission();
}

void MCP23017::writeRegister(MCP23017Register reg, u8_t portA, u8_t portB)
{
	_bus->beginTransmission(_deviceAddr);
	_bus->write(static_cast<u8_t>(reg));
	_bus->write(portA);
...
u8_t MCP23017::readRegister(MCP23017Register reg)
{
	_bus->beginTransmission(_deviceAddr);
	_bus->write(static_cast<u8_t>(reg));
	_bus->endTransmission();
	_bus->requestFrom(_deviceAddr, (u8_t)1);
	return _bus->read();
}

void MCP23017::readRegister(MCP23017Register reg, u8_t& portA, u8_t& portB)
{
	_bus->beginTransmission(_deviceAddr);
	_bus->write(static_cast<u8_t>(reg));
	_bus->endTransmission();
	_bus->requestFrom(_deviceAddr, (u8_t)2);
	portA = _bus->read();
	portB = _bus->read();
}
...
{
	u8_t iocon = readRegister(MCP23017Register::IOCON);
	if(intMode == MCP23017InterruptMode::Or) iocon |= static_cast<u8_t>(MCP23017InterruptMode::Or);
	else iocon &= ~(static_cast<u8_t>(MCP23017InterruptMode::Or));

	writeRegister(MCP23017Register::IOCON, iocon);
}

void MCP23017::interrupt(MCP23017Port port, u8_t mode)
{
...
void MCP23017::interruptedBy(u8_t& portA, u8_t& portB)
{
...
void MCP23017::clearInterrupts(u8_t& portA, u8_t& portB)
{
