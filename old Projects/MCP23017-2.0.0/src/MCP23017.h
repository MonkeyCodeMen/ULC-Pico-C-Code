 ...
 */
enum class MCP23017InterruptMode : u8_t
{
 ...
 */
enum class MCP23017Register : u8_t
{
...
inline MCP23017Register operator+(MCP23017Register a, MCP23017Port b) {
	return static_cast<MCP23017Register>(static_cast<u8_t>(a) + static_cast<u8_t>(b));
};
...
public:
	 ...
	 */
	MCP23017(u8_t address, TwoWire& bus = Wire);
	~MCP23017();
#ifdef _DEBUG
	void debug();
#endif
	 ...
	 */
	void portMode(MCP23017Port port, u8_t directions, u8_t pullups = 0xFF, u8_t inverted = 0x00);
	/**
	 ...
	 */
	void pinMode(u8_t pin, u8_t mode, bool inverted = false);
	 ...
	 */
	void digitalWrite(u8_t pin, u8_t state);
	/**
	 ...
	 * See "3.5.10 Port register".
	 */
	void writePort(MCP23017Port port, u8_t value);
	/**
	 ...
	 * See "3.5.10 Port register".
	 */
	u8_t readPort(MCP23017Port port);
	/**
	 ...
	 */
	void writeRegister(MCP23017Register reg, u8_t value);
	/**
	 ...
	 */
	void writeRegister(MCP23017Register reg, u8_t portA, u8_t portB);
	/**
	 * Reads a single register value.
	 */
	u8_t readRegister(MCP23017Register reg);
	/**
	 ...
	 * will be reversed due to the way the MCP23017 works in Byte mode.
	 */
	void readRegister(MCP23017Register reg, u8_t& portA, u8_t& portB);
	 ...
	 */
	void interrupt(MCP23017Port port, u8_t mode);
	/**
	 ...
	 */
	void interruptedBy(u8_t& portA, u8_t& portB);
	/**
	 ...
	 */
	void clearInterrupts(u8_t& portA, u8_t& portB);
