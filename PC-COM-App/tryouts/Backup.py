...
class App:
    ...
    def send_command(self,command_frame):
        """
        Send a command to the Pico and wait for the response.

        Args:
            command_frame (str): The command frame to send.
            serial_port (Serial): The serial port instance for communication.

        Returns:
            str: The response frame from the Pico or an error message.
        """
        # Frame validation: Ensure the command has a valid structure
        if not command_frame.startswith("S:") or not command_frame.endswith("#"):
            return "Error: Invalid frame structure. Frame must start with 'S:' and end with '#'"

        try:
            # Send the command
            self.serial_port.write(command_frame.encode('utf-8'))
            print(f"Sent: {command_frame}")
            ...
            while True:
                ...
                if self.serial_port.in_waiting > 0:
                    ...
                    # Check if the response contains the end-of-frame marker
                    if "#" in response_buffer:
                        break

            # Frame validation: Ensure the response has a valid structure
            if not response_buffer.startswith("A:") or not response_buffer.endswith("#"):
                return f"Error: Invalid response structure: {response_buffer}"

            print(f"Received: {response_buffer}")
            return response_buffer

        except Exception as e:
            return f"Error: Communication failed. {str(e)}"
