...
typedef void (*pixel_callback)(int16_t x, int16_t y, u8_t red, u8_t green, u8_t blue);
...
private:
    ...
    void copyImageDataRect(u8_t *dst, u8_t *src, int x, int y, int width, int height);
    void fillImageData(u8_t colorIndex);
    void fillImageDataRect(u8_t colorIndex, int x, int y, int width, int height);
    ...
    int lzw_decode(u8_t *buf, int len, u8_t *bufend);
    void lzw_setTempBuffer(u8_t * tempBuffer);
    ...
    u8_t imageData[maxGifWidth * maxGifHeight];

    // Backup image data buffer for saving portions of image disposal method == 3
    u8_t imageDataBU[maxGifWidth * maxGifHeight];
    ...
    u8_t stack  [LZW_SIZTABLE];
    u8_t suffix [LZW_SIZTABLE];
    u16_t prefix [LZW_SIZTABLE];
